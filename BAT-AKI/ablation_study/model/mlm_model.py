...
class AKIEmbeddings(nn.Module):
    def __init__(self, vocab_size, embedding_dim=256, onto_dim=64,mronto_dim=64, dropout_prob=0.1, num_ontology=3000, num_mrontology=3000, use_ontology=False, use_mrontology=False):
        super().__init__()
        ...
        if self.use_mrontology:
            self.module_ontology_embeddings = nn.Embedding(num_mrontology, mronto_dim, padding_idx=0)
    ...
    def forward(self, input_ids, ontology_ids=None, mrontology_ids=None):
        ...
        if self.use_ontology:
            assert ontology_ids is not None, "  use_ontology but need ontology_ids"
            x_onto = self.ontology_embeddings(ontology_ids)
            pieces.append(x_onto)

        if self.use_mrontology:
            assert mrontology_ids is not None, "  use_mrontology but need mrontology_ids"
...
class MaskedLanguageModel(nn.Module):
    def __init__(self, vocab_size, embedding_dim=128, max_len=500, num_heads=4, hidden_dim=512, num_layers=2, dropout=0.1, max_timescale=1e4, use_module_embedding=True, use_semantic_embedding=False, semantic_matrix=None, freeze_semantic=True, use_ontology=False, num_ontology=3000, use_mrontology=False, num_mrontology=3000):
        super().__init__()
        ...
        if self.use_semantic_embedding and semantic_matrix is not None:
            self.semantic_embedding = SemanticCodeEmbedding(semantic_matrix, freeze=freeze_semantic)
        ...
        self.encoder_layers = nn.ModuleList([
            copy.deepcopy(CustomTransformerEncoderLayer(
                d_model=model_dim,
                nhead=num_heads,
    ...
    def forward(self, input_ids, attention_mask, delta_t, segment_ids, module_ids=None, ontology_ids=None, mrontology_ids=None):
        ...
        if self.use_ontology or self.use_mrontology:        
            if self.use_ontology:
                assert ontology_ids is not None, "  use_ontology but no ontology_ids"
            if self.use_mrontology:
                assert mrontology_ids is not None, "  use_mrontology but no mrontology_ids"
        ...
        if self.use_semantic_embedding:
            x_sem = self.semantic_embedding(input_ids, attention_mask=attention_mask) 
            x_token = torch.cat([x_token, x_sem], dim=-1)
        
        x_time = self.time_embedding(delta_t, attention_mask=attention_mask)                          
        x_seg = self.seg_embedding(segment_ids, attention_mask=attention_mask)
        x = x_token + x_time + x_seg
        
        if self.use_module_embedding and module_ids is not None:
            x_module = self.module_embedding(module_ids, attention_mask=attention_mask)    
            x = x + x_module
        ...
        for layer in self.encoder_layers:
            x, attn_weights = layer(x, src_key_padding_mask=~attention_mask)
            attention_weights_all.append(attn_weights)

        logits = self.classifier(x)
        return logits, attention_weights_all, x