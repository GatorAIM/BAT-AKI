...
class TransformerForSequenceClassificationAttn(nn.Module):
    def __init__(self, pretrained_model, hidden_dim=128, freeze_encoder=False):
        super().__init__()
        ...
        self.attn_head = AttentionPoolingHead(pretrained_model.model_dim)
    ...
    def forward(self, input_ids, attention_mask, delta_t, segment_ids, module_ids=None, ontology_ids=None, mrontology_ids=None):
        if self.use_ontology or self.use_mrontology:
            if self.use_ontology:
                assert ontology_ids is not None, "use_ontology=True but ontology_ids is None"
            if self.use_mrontology:
                assert mrontology_ids is not None, "use_mrontology=True but mrontology_ids is None"
        ...
        return self.attn_head(x, attention_mask)
