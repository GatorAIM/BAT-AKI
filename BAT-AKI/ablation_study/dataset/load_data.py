...
def drop_first_token_flexibly(df):
    ...
    if "TOKEN_SEQUENCE" in df.columns:
        df["TOKEN_SEQUENCE"] = df["TOKEN_SEQUENCE"].apply(lambda x: " ".join(x.split(" ")[1:]) if isinstance(x, str) else x)

    list_fields = ["MODULE_ID_SEQUENCE", "DELTA_T_SEQUENCE", "TYPE_SEQUENCE"]
    for field in list_fields:
        if field in df.columns:
            df[field] = df[field].apply(lambda x: x[1:] if isinstance(x, list) and len(x) > 1 else x)
...
def load_ontology_map_table(file_path, token2id, filename="df_tokens_final_UMSL.pkl"):
    ...
    print(f"  ontology_map_table        : {pickle_path}")
