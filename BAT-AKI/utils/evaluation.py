...
def forward_nomodule_noonto(model, batch, device):
    ...
    logits, abnormal_logits, _, hidden = core_model(input_ids, attention_mask, delta_t, segment_ids)
    return logits, abnormal_logits, hidden, attention_mask
...
def forward_nomodule_onto(model, batch, device):
    ...
    logits, abnormal_logits, _, hidden = core_model(input_ids, attention_mask, delta_t, segment_ids, ontology_ids=ontology_ids, mrontology_ids=mrontology_ids )
    return logits, abnormal_logits, hidden, attention_mask
...
def forward_module_noonto(model, batch, device):
    ...
    logits, abnormal_logits, _, hidden = core_model(input_ids, attention_mask, delta_t, segment_ids, module_ids=module_ids)
    return logits, abnormal_logits, hidden, attention_mask
...
def forward_module_onto(model, batch, device):
    ...
    logits, abnormal_logits, _, hidden = core_model(
        input_ids, attention_mask, delta_t, segment_ids,
        module_ids=module_ids, ontology_ids=ontology_ids, mrontology_ids=mrontology_ids 
    )
    return logits, abnormal_logits, hidden, attention_mask

def evaluate_loss(model, val_loader, loss_fn, device, vocab_size=None, use_module_emb=False, use_ontology=False, do_flag2=False, use_token2weight=False, token2weight=None, use_abnormal_loss=False):
    model.eval()
    model.eval()
    total_loss = 0
    core_model = model.module if isinstance(model, torch.nn.DataParallel) else model
    ...
    with torch.no_grad():
        for batch in val_loader:
            if use_module_emb and use_ontology:
                logits, abnormal_logits, hidden, attention_mask = forward_module_onto(model, batch, device)
            elif use_module_emb and not use_ontology:
                logits, abnormal_logits, hidden, attention_mask = forward_module_noonto(model, batch, device)
            elif not use_module_emb and use_ontology:
                logits, abnormal_logits, hidden, attention_mask = forward_nomodule_onto(model, batch, device)
            else:
                logits, abnormal_logits, hidden, attention_mask = forward_nomodule_noonto(model, batch, device)

                
            labels = batch['labels'].to(device)
            
            attention_mask = batch['attention_mask'].to(device)
            if use_abnormal_loss:
                abnormal_flags = batch['abnormal_flags'].to(device)  
            ...
            if use_abnormal_loss:
                abnormal_loss = F.binary_cross_entropy_with_logits(
                    abnormal_logits, abnormal_flags, reduction='mean'
                )
                mlm_loss = mlm_loss + abnormal_loss  
