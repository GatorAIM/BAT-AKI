...
def build_split_ids_from_csv(
    ...
    def load_split(name):
        path = os.path.join(split_load_path, f"./")
...
class LSTMWithWeight(RNN):
    ...
    def forward(self, **kwargs):
        ...
        if logits is None:
            logits = out.get("pred", None)
        if logits is None:
            raise ValueError("Missing logit or pred in model output")

        if "label" in kwargs:
            device = next(self.parameters()).device
            label = torch.tensor(kwargs["label"], dtype=torch.float32, device=device).unsqueeze(-1)
            out["loss"] = self.loss_fn(logits, label)
        return out
...
def train_lstm_earlystop(
    ...
    for unit in unit_list:
        ...
        neg = sum(s["label"] == 0 for s in train_samples)
        pos = sum(s["label"] == 1 for s in train_samples)
        pos_weight = None if pos == 0 else torch.tensor([neg / max(pos, 1)], dtype=torch.float, device=device)

        for rerun in range(reruns):
            seed = base_seed + rerun
            set_seed(seed)

            model = LSTMWithWeight(
                dataset=train_ds,
                feature_keys=["code"],
                label_key="label",
                mode="binary",
